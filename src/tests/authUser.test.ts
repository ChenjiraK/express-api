...
const mockToken =
  'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpZCI6MiwiZW1haWwiOiJ0ZXN0MkBlbWFpbC5jb20iLCJpYXQiOjE3MzMxMzM3OTh9.ZzBEFVRMcOz9PQ2hFtBggHm3JPzDw0KQ7iblbZqmZ_o';
...
describe('login', async () => {
  ...
  it('should return status 200 if login successfully', async () => {
    ...
    const findOneSpy = vi
      .spyOn(User, 'findOne')
      .mockResolvedValue(mockUser as any); // get data success
    const comparePasswordSpy = vi
  ...
  it('should return status 404 if user not found', async () => {
    const req = {
      body: { email: 'nonExistingUser@email.com', password: 'testPassword' },
    } as Request;
  ...
  it('should return status 401 if password is incorrect', async () => {
    ...
    const comparePasswordSpy = vi
      .spyOn(authUtil, 'comparePassword')
      .mockResolvedValue(false); // return password not match
...
describe('register', async () => {
  ...
  it('should return status 201 if register successfully', async () => {
    const getRequestParamsSpy = vi
      .spyOn(User, 'getRequestParams')
      .mockResolvedValue(mockRequestData as any); //get params before send api
    const checkUserExistsSpy = vi
      .spyOn(authUtil, 'checkUserExists')
      .mockResolvedValue(null); //ไม่มีค่าซ้ำ
    const createSpy = vi
      .spyOn(User, 'create')
      .mockResolvedValue(mockResponseData as any); //mock response from api

    await register(req, res);

    expect(getRequestParamsSpy).toHaveBeenCalledWith(req.body);
    expect(checkUserExistsSpy).toHaveBeenCalledWith(
      null,
      req.body.username,
      req.body.email
    );
  ...
  it('should return status 409 if email or username already exists', async () => {
    const getRequestParamsSpy = vi
      .spyOn(User, 'getRequestParams')
      .mockResolvedValue(mockRequestData as any);
    const checkUserExistsSpy = vi
      .spyOn(authUtil, 'checkUserExists')
      .mockResolvedValue('this username already exists'); //return text from checkUserExists
